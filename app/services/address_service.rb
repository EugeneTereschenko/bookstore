class AddressService
  ...
  def save(user, address_params)
    ...
    @address_params[:order_id] = 0
    @checkbox = @address_params[:checkbox_id]
    ...
    else @errors.push('Unknown type of address.')
    end
  ...
  def billing_address(user)
    Address.find_by(user_id: user.id, order_id: 0, address_type: 'billing') || Address.new
  end

  def shipping_address(user)
    Address.find_by(user_id: user.id, order_id: 0, address_type: 'shipping') || Address.new
  end
end