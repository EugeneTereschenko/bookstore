class CheckoutStepsController < ApplicationController
  ...
  def show
    ...
    when :checkout_address
      if Item.count.positive?
        ...
        flash[:alert] = "Your cart is empty now"
    ...
    when :checkout_payment
      if current_state == 'checkout_address'
        flash[:alert] = "Sorry, it seems that you jumped over step"
        redirect_back(fallback_location: root_path) && return
      end
      set_checkout_step(step)
  ...
  def set_checkout_step(step)
    ...
    flash[:alert] = "Error save checkout state"
    redirect_back(fallback_location: root_path) && return
  end
  ...
  def choose_delivery
    ...
    if errors.nil?
      ...
      flash[:alert] = "Error add delivery!"
      redirect_back(fallback_location: root_path) && return
    end
  end
  ...
  def apply_card
    errors = card_service.save_card(checkout_params)
      ...
      render_wizard
    #else
     # flash[:alert] = "Enter valid values!"
    #  redirect_back(fallback_location: root_path) && return
    #end
  end
  ...
  def create_order
   ...
   # if errors.nil?
      ...
      clean_cart
    #else
    #  flash[:alert] = "Error created order!"
   # end
  end
